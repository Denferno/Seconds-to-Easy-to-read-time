...
def format_duration(seconds):
    ...
    for time in duration_list_s:  # the reason it's called duration_called_s is because
        if re.match('1\s', time):  # the string always ends with s even though its just one number (1 seconds)
            duration_list.append(
                time[:-1])  # the [:-1] excludes the last letter, so if it start with zero, for example
    ...
    if len(duration_list) == 1:  # depending on how many times are in the list we need to add 'and' and ','
        ...
        return (
            f'{duration_list[0]}, {duration_list[1]}, {duration_list[2]}, {duration_list[3]} and {duration_list[4]}')

while isinstance(seconds, int) == False:
    try:
        seconds = int(input('Type the amount of seconds you would like to convert to a readable sentence: '))
    except ValueError:
        print("Program is not able to run, because you did not type a number")
    except:
        print('Program is not able to run')
